...
export default function LessonScreen(): React.JSX.Element {
  ...
  const studentName = profile.name || 'Friend';
  const interestKey = profile.interests && profile.interests.length > 0 ? profile.interests[0] : 'default';
  
  const storyTemplate = currentTopic.base_story_template[language] || currentTopic.base_story_template.en;
  const interestPlace = (currentTopic.interest_placeholders.INTEREST_PLACE as any)[interestKey]?.[language] ||
                        (currentTopic.interest_placeholders.INTEREST_PLACE as any)[interestKey]?.en ||
                        (currentTopic.interest_placeholders.INTEREST_PLACE as any)['default']?.[language] ||
                        (currentTopic.interest_placeholders.INTEREST_PLACE as any)['default']?.en;

  const resolvedStory = storyTemplate
